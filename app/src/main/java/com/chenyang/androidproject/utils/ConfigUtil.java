 ...
 * 以确保和{@link PreferenceFragmentCompat}操作同一个xml。
 */
public class ConfigUtil {
     ...
     * 图像质量检测阈值
     */
    private static final float RECOMMEND_IMAGE_QUALITY_THRESHOLD = 0.35f;
    /**
     * 默认最大人脸检测数量
     */
    ...
    public static float getImageQualityThreshold(Context context) {
        return Float.parseFloat(getString(context, R.string.preference_image_quality_threshold, String.valueOf(RECOMMEND_IMAGE_QUALITY_THRESHOLD)));
    }
    ...
    public static boolean isEnableImageQualityDetect(Context context) {
        return getBoolean(context, R.string.preference_enable_image_quality_detect, false);
    }
    ...
    public static String getAppId(Context context) {
        return getString(context, R.string.preference_app_id, FaceActivity.APP_ID);
    }

    public static String getSdkKey(Context context) {
        return getString(context, R.string.preference_sdk_key, FaceActivity.SDK_KEY);
    }

    public static String getActiveKey(Context context) {
        return getString(context, R.string.preference_active_key, FaceActivity.ACTIVE_KEY);
    }
